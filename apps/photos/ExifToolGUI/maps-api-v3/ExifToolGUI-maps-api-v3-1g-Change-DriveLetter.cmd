...
call :getDriveLetter ":\apps\unix\msys64\usr\bin\hexdump.exe" hexdump
call :getDriveLetter ":\apps\unix\msys64\usr\bin\dd.exe" dd
...
if %_Debug%==1 (
   ...
   echo  _file_html: %_file_html%
   echo.
)


call :ExifToolGUIChangeDriveLetterInhtml %_file_html% %_DRV%
rem call :ExifToolGUIChangeDriveLetterInExe %_file_exe% %_DRV%
...
::
for /f "tokens=3" %%a in ('"%hexdump% -n 1 -C -s 0x9A %file_html_in%"') do ( set "DriveLetter_file_in=%%a" )
set DriveLetter_file_in=%DriveLetter_file_in:~1,1%
...
::
for /f "tokens=3" %%a in ('"%hexdump% -n 1 -C -s 0xCC %file_html_in%"') do ( set "DriveLetter_file_in=%%a" )
set DriveLetter_file_in=%DriveLetter_file_in:~1,1%
...
if not "%DRV_out%"=="%DriveLetter_file_in%" (
	...
	copy %file_html_in% %file_html_in%.bak

	echo --- 1er DriveLetter offset: 0x9A = 154 - file:///D:/apps/photos/ExifToolGUI/ & echo.
	::
	%hexdump% -n 27 -C -s 0x9A %file_html_in%
	echo.
	echo %DRV_out%| %dd% of="%file_html_in%" bs=1 seek=154 count=1 conv=notrunc
	echo.
	%hexdump% -n 27 -C -s 0x9A %file_html_in%

	echo.&echo.

	echo --- 2o  DriveLetter offset: 0xCC = 204 - file:///D:/apps/photos/ExifToolGUI/maps-api-v3/js.js & echo.
	::
	%hexdump% -n 44 -C -s 0xCC %file_html_in%
	echo.
	echo %DRV_out%| %dd% of="%file_html_in%" bs=1 seek=204 count=1 conv=notrunc
	echo.
	%hexdump% -n 44 -C -s 0xCC %file_html_in%

) else ( echo file: %file_html_in% ya esta cambiado 1er DriveLetter: %DriveLetter_file_in% & echo. )
...
::
for /f "tokens=3" %%a in ('"%hexdump% -n 1 -C -s 0x252D24 %file_exe_in%"') do ( set "DriveLetter_file_in=%%a" )
set DriveLetter_file_in=%DriveLetter_file_in:~1,1%
...
::
for /f "tokens=3" %%a in ('"%hexdump% -n 1 -C -s 0x252D56 %file_exe_in%"') do ( set "DriveLetter_file_in=%%a" )
set DriveLetter_file_in=%DriveLetter_file_in:~1,1%
...
if not "%DRV_out%"=="%DriveLetter_file_in%" (
	...
	echo --- 1er DriveLetter  offset: 0x252D24 = 2436388 - file:///D:/apps/photos/ExifToolGUI/ & echo.
	::
	%hexdump% -n 27 -C -s 0x252D24 %file_exe_in%
	echo.
	echo %DRV_out%| %dd% of="%file_exe_in%" bs=1 seek=2436388 count=1 conv=notrunc
	echo.
	%hexdump% -n 27 -C -s 0x252D24 %file_exe_in%

	echo.&echo.

	echo --- 2o  DriveLetter offset: 0x252D56 = 2436438 - file:///D:/apps/photos/ExifToolGUI/maps-api-v3/js.js & echo.
	::
	%hexdump% -n 44 -C -s 0x252D56 %file_exe_in%
	echo.
	echo %DRV_out%| %dd% of="%file_exe_in%" bs=1 seek=2436438 count=1 conv=notrunc
	echo.
	%hexdump% -n 44 -C -s 0x252D56 %file_exe_in%

) else ( echo file: %file_exe_in% ya esta cambiado 1er DriveLetter: %DriveLetter_file_in% & echo. )
